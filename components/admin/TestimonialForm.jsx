...
import { stripHtml } from '@/util/wordUtils';
      ...
      <div>
        ...
        <select name="course_id" value={form.course_id} onChange={handleChange} required>
          <option value="">Select Course</option>
              {courses.map((c) => (
                <option value={c.id} key={c.id}>{stripHtml(c.title)}</option>
              ))}
        </select>
        {errors.course_id && <span className="err">{errors.course_id}</span>}
      </div>
          {/* Preview the selected course title rendered as HTML */}
          {form.course_id && (
            <div className="course-preview" aria-live="polite">
              <small style={{ color: '#666' }}>Selected course:</small>
              <div dangerouslySetInnerHTML={{ __html: (courses.find(c => String(c.id) === String(form.course_id))?.title) || '' }} />
            </div>
          )}
      <div>
        <label>Content</label>
