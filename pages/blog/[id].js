...
export default function BlogDetails() {
    ...
    const metaTitle = blogPost && blogPost.title ? `${stripTags(blogPost.title)} | ${siteName}` : `${siteName} - Blog`
    const metaDescription = stripTags(blogPost.excerpt || blogPost.content || '').slice(0, 160)
    const metaOgImage = blogPost.featured_image_path ? `${process.env.NEXT_PUBLIC_API_BASE_URL}/storage/${blogPost.featured_image_path}` : '/assets/img/blog/blog_thumb07.jpg'
    const metaCanonical = `${siteUrl}/blog/${id}`
